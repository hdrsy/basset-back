...
module.exports = {
    async getSecondCommitmentQuestion(ctx){

        const { contract_id } = ctx.request.body;
        console.log('contract_id')
        console.log(contract_id)
        // let types = strapi.query('api::type.type').find();
        // get the second commitment question of specified target that answered of the contract
        ...
        let middles = await strapi.entityService.findMany('api::middle.middle', {
          ...
          console.log('secondcommitments');
          console.log(middles[0].secondcommitment);
         
          // return types;
          let answers_array = [];
          let questions_array = [];
    
          middles[0].secondcommitment.forEach(element => {
                let answer_item = {
                    id : element.id,
                    text : element.text,
                }
                answers_array.push(answer_item);
                element.secondcommitmentQuestion.forEach(item => {
                    let question_item= {
                      id : item.id,
                      text : item.text,
                      type:item.type,
...
// answer second commitments question  
    async answerSecondCommitmentQuestion(ctx){
        ...
        const {answer_array,contract_id } = ctx.request.body;
            ...
            const responseData = {
              data : "",
              statusCode: success_status,
              message: "secondcommitment successfully answered"
            }
            return responseData;  
